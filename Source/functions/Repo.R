...
###########################################################################
if (!exists("Repo") || is.environment(Repo)) Repo <- new.env(parent = emptyenv())
local({
    .VERSION = "0.31"

    .LOGFILE <- stdout()
    catLog <- function(...) { cat(format(Sys.time(), "[%Y-%m-%d %H:%M:%S %z]"), ...) }
    
    #' Retrieve the sample information from E.xxx/info/xxxxx.txt
    #' @EID experiment ID
    #' @SID sample ID
    getSampleInfo <- function(EID, header, base = "/lab/repo") {
        EID1 <- paste("E", EID, sep = ".")
        catLog("Processing", EID1, "...\n", file = .LOGFILE)
        dir.exp <- file.path(base, EID1, "info")
        fileSampleInfo <- list.files(dir.exp, pattern = ".txt")
        if(length(fileSampleInfo) > 1) {
            stop("Multiple found: ", filesampleInfo)
        } 
        fileSampleInfo <- file.path(dir.exp, fileSampleInfo)
        if (.DEBUG) catLog("Reading", fileSampleInfo, "...\n", file = .LOGFILE)
        sampleInfo <- read.csv(fileSampleInfo, head = F, sep = "\t", as.is = T)
        if (length(header) != ncol(sampleInfo)) {
            stop("Not Matched: ", length(header), " ", ncol(sampleInfo))
        }
        colnames(sampleInfo) <- header
        sampleInfo
    }

    getVerseGTF <- function(EID, SID, base = "/lab/repo") {
        file.exp <- paste("E", EID, sep = ".")
        file.sample <- paste("Sample", gsub(" ", "", SID), sep = "_")
        file.log <- paste(file.sample, "verse", "log.txt", sep = ".")
        filename <- file.path(base, file.exp, "analyzed", file.sample, "verse", 
                              file.log)
        if (.DEBUG) catLog(filename, "\n", file = .LOGFILE)
        if (file.exists(filename)) {
            texts <- readLines(filename)
            text <- texts[32]
            if (.DEBUG) catLog(text, "\n", file = .LOGFILE)
            annot <- gsub("\\|\\| Load annotation file (.+\\.gtf).*\\|\\|", 
                          "\\1", text, perl = T)
        } else {
    ...
    getVERSEcounts <- function(exptName, sampleID, baseDir = "/lab/repo", analyzedDir = "analyzed", countsDir = "verse", fileBase = "verse", features = c("exon", "exon-lev3", "anti-exon", "intron", "intron-lev1-lev2", "intron-lev3", "anti-intron", "intergenic", "lines_sines", "mito", "anti-mito")) {
    ...
    getSTARstatsFromChex <- function(EID, SID, base = "/lab/repo", analyzedDir = "analyzed", starDir = "star", statsFile = NULL) {
        .LOGFILE <- stderr()
        catLog <- function(...) { cat(format(Sys.time(), "[%Y-%m-%d %H:%M:%S GMT%z]"), ...) }
        Ename <- paste("E", EID, sep = ".")
        Sname <- paste("Sample", gsub(" ", "", SID), sep = "_")
        fname_version <- paste(Sname, "versions", sep = ".")
        fname_stats <- if (is.null(statsFile)) paste(Sname, "star", "stats.txt", sep = ".") else statsFile
        filename_version <- file.path(base, Ename, analyzedDir, Sname, starDir, fname_version)
        filename_stats <- file.path(base, Ename, analyzedDir, Sname, starDir, fname_stats) 
        if (.DEBUG) catLog("versionFile: ", filename_version, "\n", file = .LOGFILE)
        if (file.exists(filename_version)) {
            sect1 <- as.matrix(read.table(filename_version, sep = "\t", as.is = T, check = F))[2, -c(2, 4)]
            ver <- sub("-\\\\w+$", "", sect1[1])
            #' Pipeline version 2.0 has the issue that "SE?" and "readLength" are switched
        ...
        names(sect1) <- c( 
            "pipeline", "star.version", "samtools.version", 
            "species (genome)", "SE", "readLength")
        if (.DEBUG) catLog("sect1: ", sect1, "\n", file = .LOGFILE)
        if (.DEBUG) catLog("statsFile: ", filename_stats, "\n", file = .LOGFILE)
        if (file.exists(filename_stats)) {
            x <- read.delim(filename_stats, head = F, as.is = T)
            sect2 <- c(
                as.numeric(x[grep("Number of input reads", x[, 1]), 2]), 
        ...
        if (.DEBUG) catLog("sect2: ", sect2, "\n", file = .LOGFILE)
        c(sect1, sect2)
    }
    #' To be implemented
    getHTSEQstats_perFeature <- function(feature, base = "/lab/repo", Ename, Sname) {
        ...
    }
    
    #' To be implemented
    getHTSEQstats <- function(EID, SID, base = "/lab/repo", features = c( "exon", "exon-lev3", "anti-exon", "intron-lev1-lev2", "intron-lev3", "intron", "anti-intron", "mito", "anti-mito", "intergenic", "lines_sines")) {
        ...
    }
    ...
    GTExSampleInfoHeader <- c("Assay_Type_s", "AssemblyName_s", "BioSample_s", "Center_Name_s", "Experiment_s", "InsertSize_l", "LibraryLayout_s", "LibrarySelection_s", "LibrarySource_s", "Library_Name_s", "LoadDate_s", "MBases_l", "MBytes_l", "ReleaseDate_s", "Run_s", "SRA_Sample_s", "Sample_Name_s", "analyte_type_s", "biospecimen_repository_sample_id_s", "body_site_s", "data_type_s", "flowcell_barcode_s", "histological_type_s", "instrument_name_s", "is_tumor_s", "lane_s", "library_type_s", "molecular_data_type_s", "product_order_s", "product_part_number_s", "run_barcode_s", "run_name_s", "sex_s", "submitted_subject_id_s", "work_request_s", "BioProject_s", "Consent_s", "Organism_s", "Platform_s", "SRA_Study_s", "biospecimen_repository_s", "g1k_analysis_group_s", "g1k_pop_code_s", "gap_accession_s", "source_s", "study_design_s", "study_name_s")

    checkoutGTEx <- function(EID, outDir, features = c("exon", "mito"), base = "/lab/repo", header = GTExSampleInfoHeader) {
        ENames <- paste0("E.", EID)
        #' Read the sample info
        sampleInfo <- sapply(EID, getSampleInfo, header = header, simplify = F)
        sampleInfo <- do.call(rbind, sampleInfo)
        #' Read the alignment stats
        SIDs <- sampleInfo[, "Run_s"]
        stats <- t(sapply(SIDs, function(SID) getNGSstats(EID = EID, SID = SID, features = c( "exon", "exon-lev3", "anti-exon", "intron-lev1-lev2", "intron-lev3", "intron", "anti-intron", "mito", "anti-mito", "intergenic", "lines_sines"))))
        stats <- cbind("RepoID" = EID, "SampleID" = rownames(stats), stats)

        metadata <- merge(x = sampleInfo, y = stats, by.x = "Run_s", by.y = "SampleID")
        idx <- which(colnames(metadata) == "Run_s")
        colnames(metadata)[idx] <- "SampleID"
        metadata <- metadata[order(metadata[, "SampleID"]), ]
        idx <- which(colnames(metadata) == "RepoID")
        metadata <- metadata[, c(idx, 1:(idx-1), (idx+1):ncol(metadata))]
        dir.create(outDir, F, T)
        filenameMetadata <- sprintf("metadata_E%s.csv.gz", EID)
        filenameMetadata <- file.path(outDir, filenameMetadata)
        write.csv(metadata, file = gzfile(filenameMetadata), row.names = F, na = "")
        #' Read the counts data: features are "exon" and "mito"
        counts <- lapply(1:nrow(metadata), function(i) 
                         getVERSEcounts(metadata[i, "RepoID"], 
                                        metadata[i, "SampleID"], 
                                        features = features))
        SIDs <- metadata[, "SampleID"]
        names(counts) <- SIDs
        is_empty <- sapply(counts, function(x) nrow(x[[1]]) == 0)
        if (any(is_empty)) { 
            catLog("Empty samples:\n", file = .LOGFILE)
            catLog(SIDs[is_empty], "\n", file = .LOGFILE)
        }
        SIDs <- SIDs[!is_empty]
        counts <- counts[!is_empty]
        if (length(SIDs) < 1) {
            catLog("No non-empty samples, exiting...\n", file = .LOGFILE)
            stop()
        }
        for (feature in features) {
            cnts <- lapply(counts, function(x) x[[feature]])
            #' Make sure all features are identical across samples
            genes <- sapply(cnts, function(x) rownames(x))
            #' Get feature IDs
            genes <- apply(genes, 1, unique)
            if (!is.atomic(genes)) {
                catLog("Inconsistent gene IDs, exiting...\n", file = .LOGFILE)
                stop()
            }
            #' Extract the count matrix
            cnts <- sapply(cnts, function(cnts) cnts[genes, ])
            rownames(cnts) <- genes
            catLog("Dim of", feature, dim(cnts), "\n", file = .LOGFILE)
            filenameCounts <- sprintf("counts_%s_E%s.csv.gz", feature, EID)
            filenameCounts <- file.path(outDir, filenameCounts)
            catLog("Saving", filenameCounts, "...\n", file = .LOGFILE)
            write.csv(cnts, file = gzfile(filenameCounts))
        }
    }
